...
for (const size of ['mini']) for (const hero of ['jessie', 'jane']) {
...
console.log(`PASS: ${Object.keys(baseline.files).length} approved master hashes unchanged; ${rendered.length} native scene probes; opaque palette pixels, no sprite scaling, integer origins, and the mini family.`);
