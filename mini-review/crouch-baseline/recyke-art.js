...
(() => {
  ...
  function actors(c,s) {
    ...
    else frame=frameFor(M,state,p.stateTime,['fire','jab','hurt'].includes(state));
    M.draw(c,s.size,Math.round(p.x),Math.round(p.y),{state,frame,time:p.stateTime||0,facing:p.facing||1});
    if((p.invulnerable||p.inv||0)>0&&Math.floor(s.time*8)%2===0){const spread=small?8:20,top=p.y-(small?33:98);r(c,'cream',p.x-spread,top,3,1);r(c,'cream',p.x+spread-3,top,3,1);}
  }
  function shots(c,s) {
    const small=s.size==='mini';
