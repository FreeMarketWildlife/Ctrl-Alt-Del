(() => {
  ...
  const aliases={'jessie-detailed':'jessie-big',jessie:'jessie-mini','jessie-siege':'jessie-big'};
  const motionNames={idle:'Ready',walk:'Walk',run:'Run',fire:'Fire', 'run-fire':'Run + fire',jump:'Jump','jump-fire':'Jump + fire',crouch:'Crouch',jab:'Jab',hurt:'Hit reaction'};
  ...
  function select(item){
    ...
    const states=item.jessie?CADJessie.states:item.detailed?CADDetailedCharacters.states:item.group==='Characters'?['idle','run','jump','fire','punch','hurt']:item.group==='Worlds'?['ambient']:item.group==='Weapons'?['idle','fire']:item.group==='Tools'?['idle','run']:['idle','run','fire'];
    motion.replaceChildren(...states.map(state=>{
      const option=document.createElement('option');option.value=state;
      option.textContent=item.jessie?motionNames[state]:state==='idle'&&item.detailed?'Fighting stance':state==='run'&&item.group==='Tools'?'Boost':state.charAt(0).toUpperCase()+state.slice(1);
      return option;
    }));
    if(item.jessie)motion.value='run';else if(item.detailed)motion.value='walk';
    document.getElementById('walk-review').hidden=!item.detailed;
    document.getElementById('jessie-review').hidden=!item.jessie;
    const colors=item.jessie?CADJessie.palette:item.detailed?CADDetailedCharacters.palettes[item.hero]:[];
  ...
  function paint(){
    ...
    if(current.jessie){
      const frame=CADJessie.frameAt(motion.value,time),times=CADJessie.durations[motion.value];
  ...
  document.getElementById('step').onclick=()=>{
    ...
    if(current.jessie){
      const times=CADJessie.durations[motion.value],next=(CADJessie.frameAt(motion.value,time)+1)%times.length;
  ...
  document.getElementById('export').onclick=()=>{
    let source=canvas;
    if(current.jessie){
      const spec=CADJessie.specs[current.jessie];source=document.createElement('canvas');source.width=spec.width;source.height=spec.height;
      CADJessie.draw(source.getContext('2d'),current.jessie,spec.anchor[0],spec.anchor[1],{state:motion.value,time,facing});
    }
  ...
  document.getElementById('sheet').onclick=()=>{
    if(!(current.jessie||current.detailed))return;
    const a=document.createElement('a');a.download=current.id+'-sheet.png';
    a.href=(current.jessie?CADJessie.sheet(current.jessie):CADDetailedCharacters.sheet(current.hero)).toDataURL();a.click();
  };
