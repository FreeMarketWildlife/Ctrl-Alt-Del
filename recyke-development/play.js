(() => {
  ...
  function renderHome() {
    state.screen = "home";
    menuPanel.innerHTML = `${title("CHOOSE YOUR EPISODE", "THE RESISTANCE ARCHIVE")}
      <div class="menu-list">
        <button class="menu-btn primary" data-go="levelOne">EPISODE 1 <small>CTRL ALT DEL / PLAY</small></button>
        <a class="menu-btn concept-link" href="recyke.html?size=big&amp;hero=jessie">RECYKE / FIRST SHIFT <small>NEW STARTER LEVEL / BIG + SMALL</small></a>
        <button class="menu-btn" disabled>EPISODE 2 <small>A GLITCH IN THE SYSTEM / LOCKED</small></button>
        <button class="menu-btn" disabled>EPISODE 3 <small>CLASSIFIED / LOCKED</small></button>
        <a class="menu-btn concept-link" href="codex.html">Concept Art <small>CHARACTERS / MACHINES / WORLDS</small></a>
      </div>`;
    bindMenuButtons();
  ...
  function bindMenuButtons() {
    bindGeneric();
    buttons = [...menuPanel.querySelectorAll(".menu-btn:not(:disabled)")];
    focusIndex = 0;
    buttons[0]?.focus();
  }
  ...
  if(['fps','chase'].includes(requestedMode))window.CADPrototypes.launch(requestedMode);
  drawWallpaper();
})();
