...
(() => {
  function draw(ctx,item,t=0,state='idle',face=1){
    ...
    } else if(item.group==='Mechs'){
      CADWorld.mech(ctx,0,0,{time:step?t:0,facing:1,charging:action&&f<4});
      if(item.id==='bastion'){r('#111b2b',-17,-38,34,9);r('#788b96',-16,-38,32,2);for(let x=-13;x<15;x+=7){r('#202c42',x,-35,5,5);r('#d48b66',x+1,-34,3,2);}r('#445869',-19,-26,8,17);r('#9ba8a8',-19,-26,2,17);}
      if(item.id==='scrapper'){r('#a37752',-15,-33,9,12);r('#dda574',-15,-33,9,2);r('#414651',12,-29,9,5);r('#bf8a5d',20,-28,4,15);const claw=action?f%4:2;r('#d7b38a',17-claw,-14,5,3);r('#d7b38a',24+claw,-14,5,3);r('#2a343e',-11,-20,5,4);}
      if(action&&item.id!=='scrapper'&&f<2){r('#ffd282',30,-28,8,7);r('#fff3c9',38,-26,5,3);}
    } else if(item.group==='Drones'){
      ctx.translate(0,-18+Math.round(Math.sin(t*4)*2));const wing=item.id==='manta'?24:14;
      r('#101c2c',-wing,-7,wing*2,8);r('#5a7286',-wing+2,-7,wing*2-4,3);r('#aec2c1',-wing+3,-7,wing*2-6,1);r('#101c2c',-8,-12,16,20);r('#64778d',-7,-11,14,16);r('#24374e',-5,-8,10,9);r(item.id==='collector'?'#e4b06f':'#f06e8d',-3,-5,6,3);
      if(item.id==='manta'){r('#4a657d',-24,-3,8,7);r('#4a657d',16,-3,8,7);r('#8eacc0',-22,3,5,2);r('#8eacc0',17,3,5,2);}
      else for(const x of [-18,14]){r('#182537',x,-10,4,12);r('#96bac2',x-4+(f%2)*2,-11,12-(f%2)*4,1);r('#55c8c3',x+1,4,2,2+f%3);}
      if(item.id==='collector'){r('#ab875e',-5,8,2,8);r('#ab875e',4,8,2,8);r('#d4ae7e',-8,15,5,2);r('#d4ae7e',4,15,5,2);}
      if(action&&f<2){r('#ffb17f',-2,9,4,10);r('#fff0c7',-1,19,2,5);}
  ...
  const entries=[
    ...
    ['jane-detailed','Characters','Jane / Revision 01','Jane in a magenta field jacket, fitted combat trousers, armored knees and a swinging violet ponytail. A narrower profile with a fuller arm swing and delayed ponytail motion. Fixed 16-color palette; compare the neutral standing pose, walk and combat studies.',{hero:'jane',detailed:true}],
    ...
    ['jane','Characters','Jane / Field','Magenta jacket, agile silhouette and a swinging violet-black ponytail.',{hero:'jane'}],
    ['jane-courier','Characters','Jane / Courier','A recycled courier pack and wind-tossed scarf for the routes below Skyview.',{hero:'jane',variant:'courier'}],
    ['sentinel','Mechs','Sentinel','The patrol chassis: piston legs, a charging optic and an arm-mounted cannon.'],
    ['bastion','Mechs','Bastion','A siege chassis with a shoulder missile rack and a broad riot shield.'],
    ['scrapper','Mechs','Scrapper','A reclaimed industrial suit. Rusted armor and a powered salvage claw.'],
    ['watcher','Drones','Watcher','An exposed twin-rotor surveillance drone with a pulsing sensor.'],
    ['manta','Drones','Manta','A wide armored wing and a suspended pulse emitter for aerial interdiction.'],
    ['collector','Drones','Collector','A scrapyard drone with an amber eye and hanging retrieval claws.'],
