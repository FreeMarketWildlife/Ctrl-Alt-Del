...
(() => {
  ...
  const configs = {
    big: {
      size:'big', title:'Sorting Row', subtitle:'Recyke · Rail approach', width:640, height:360,
      worldWidth:3900, floor:308, safeUntilX:236, spawn:{x:92,y:308}, playerWidth:28, playerHeight:90,
      speed:160, acceleration:1400, gravity:900, jumpSpeed:460, maxFallSpeed:620,
      muzzleX:40, muzzleY:65, shotSpeed:640, shotRange:540, enemyShotSpeed:185,
      enemyRange:350, droneOffset:64, enemyWidth:34, enemyHeight:90, droneWidth:44, droneHeight:42,
      meleeReach:69, pickupRadius:31, maxHp:6, checkpoint:{x:2050,y:308}, exit:{x:3784,y:308},
      platforms:[
        {x:0,y:308,w:3900,h:52,kind:'ground'},
        {x:620,y:276,w:64,h:32,kind:'crate'}, {x:708,y:244,w:64,h:64,kind:'crate'},
        {x:792,y:216,w:292,h:10,kind:'platform',oneWay:true},
        {x:1190,y:292,w:256,h:16,kind:'belt'},
        {x:1496,y:276,w:64,h:32,kind:'crate'}, {x:1580,y:236,w:64,h:72,kind:'crate'},
        {x:1662,y:196,w:284,h:10,kind:'platform',oneWay:true},
        {x:2280,y:276,w:64,h:32,kind:'crate'},
        {x:2472,y:250,w:240,h:10,kind:'platform',oneWay:true},
        {x:2810,y:284,w:272,h:24,kind:'belt'},
        {x:3190,y:276,w:64,h:32,kind:'crate'}
      ],
      enemies:[
        {id:'sentinel',x:488,y:308,left:430,right:550,hp:3},
        {id:'watcher',x:1090,y:308,left:1010,right:1160,hp:3},
        {id:'scrapper',x:1390,y:292,left:1260,right:1435,hp:4},
        {id:'manta',x:1820,y:220,left:1680,right:1900,hp:3,optional:true},
        {id:'collector',x:2420,y:306,left:2350,right:2460,hp:3},
        {id:'sentinel',x:2790,y:308,left:2740,right:2800,hp:3},
        {id:'scrapper',x:3020,y:284,left:2870,right:3060,hp:4},
        {id:'bastion',x:3510,y:308,left:3420,right:3590,hp:8,gate:true}
      ],
      pickups:[{x:962,y:210,kind:'health'},{x:1340,y:283,kind:'scrap'},
        {x:1718,y:190,kind:'scrap'},{x:1844,y:190,kind:'scrap'},
        {x:2164,y:301,kind:'health'},{x:2578,y:244,kind:'scrap'},{x:3290,y:301,kind:'health'}],
      zones:[
        {x:0,name:'Sorting Row',hint:'Move through the yard. Hold fire to clear the patrol.'},
        {x:600,name:'Conveyor Crossing',hint:'Jump onto the crates. The upper bridge holds a repair kit.'},
        {x:1460,name:'Scrap Route',hint:'Climb the optional salvage walk, or continue along the floor.'},
        {x:1980,name:'Repair Station',hint:'Reach the blue beacon to save your progress and repair.'},
        {x:2720,name:'Rail Approach',hint:'The Bastion guards the depot. Its amber charge warns before a shot.'}
      ]
    },
    mini: {
      size:'mini', title:'Sorting Row', subtitle:'Recyke · Lower service lane', width:480, height:270,
      worldWidth:2112, floor:222, safeUntilX:120, spawn:{x:44,y:222}, playerWidth:10, playerHeight:30,
  ...
  function create({size='big',hero='jessie',random=Math.random}={}) {
    size=size==='mini'?'mini':'big'; let selectedHero=hero==='jane'?'jane':'jessie';
    const c=configs[size], tiny=size==='mini'; let data,uid=0,previous={jump:false,swap:false},jumpBuffer=0,coyote=0;
    let checkpointReached=false, killed=new Set(),collected=new Set();
    const rectPlayer = () => ({x:data.player.x-c.playerWidth/2,y:data.player.y-c.playerHeight,w:c.playerWidth,h:c.playerHeight});
    ...
    function reset({checkpoint=false,hero}={}) {
      ...
      data={size,view:{width:c.width,height:c.height},config:c,
        player:{...spawn,vx:0,vy:0,hp:c.maxHp,maxHp:c.maxHp,hero:selectedHero,facing:1,onGround:true,state:'idle',stateTime:0,runTime:0,invulnerable:resume?1.4:0,fireCooldown:0,meleeCooldown:0,attackTime:0,hurtTime:0},
        ...
        shots:[],pickups:c.pickups.map((p,i)=>({...p,uid:'pickup-'+i,collected:collected.has('pickup-'+i)})),platforms:clone(c.platforms),
    ...
    function damagePlayer(amount,facing) {
      ...
      p.hp=Math.max(0,p.hp-amount);p.invulnerable=1.2;p.hurtTime=.22;p.vx=(facing||-p.facing)*(tiny?35:75);setState(p,'hurt');
      data.effects.push({uid:++uid,x:p.x,y:p.y-c.playerHeight*.55,kind:'hit',life:.2,maxLife:.2});
    ...
    function shootPlayer() {
      const p=data.player, sx=p.x+p.facing*c.muzzleX,sy=p.y-c.muzzleY;
      ...
      if(p.state==='fire')p.stateTime=0; // Restart standing recoil; run phase stays continuous.
    }
    ...
    function movePlayer(dt,input) {
      ...
      const target=direction*c.speed,change=c.acceleration*dt;p.vx+=clamp(target-p.vx,-change,change);
      if(jumpBuffer>0&&coyote>0){p.vy=-c.jumpSpeed;p.onGround=false;jumpBuffer=0;coyote=0;p.jumpTime=0;}
      ...
      for(const platform of data.platforms) {
        ...
        else if(!platform.oneWay&&p.vy<0&&oldY-c.playerHeight>=platform.y+platform.h-.02&&p.y-c.playerHeight<=platform.y+platform.h){p.y=platform.y+platform.h+c.playerHeight;p.vy=0;body=rectPlayer();}
      }
      if(p.y>c.height+c.playerHeight){damagePlayer(c.maxHp,0);}
      if(input.melee&&p.meleeCooldown<=0)meleePlayer();
      ...
      if(p.hurtTime>0)setState(p,'hurt');else if(p.attackTime>0)setState(p,'jab');else if(!p.onGround)setState(p,armed?'jump-fire':'jump');else if(Math.abs(p.vx)>4)setState(p,armed?'run-fire':'run');else setState(p,armed?'fire':'idle');
      p.stateTime+=dt;
    ...
    function updateWorld(dt) {
      ...
      for(const item of data.pickups){if(item.collected)continue;const reach=c.pickupRadius;
        if(Math.abs(item.x-p.x)<reach&&item.y>=p.y-c.playerHeight-reach*.3&&item.y<=p.y+reach*.3){
      ...
      if(p.x>data.exit.x-(tiny?10:24)&&Math.abs(p.y-data.exit.y)<(tiny?35:70)){
        if(data.exit.open){data.won=true;p.vx=0;setState(p,'idle');data.notice='Sorting Row clear. The Recyke rail route is open.';data.noticeTime=100;}
