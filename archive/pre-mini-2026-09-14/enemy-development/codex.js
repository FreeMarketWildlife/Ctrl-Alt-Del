(() => {
  ...
  const aliases={'jessie-detailed':'jessie-big',jessie:'jessie-mini','jessie-siege':'jessie-big','jane-detailed':'jane-big',jane:'jane-mini',...Object.fromEntries(CADEnemies.entries.map(entry=>[entry.id,entry.id+'-big']))};
  const motionNames={stand:'Stand',idle:'Ready',walk:'Walk',run:'Run',fire:'Fire', 'run-fire':'Run + fire',jump:'Jump','jump-fire':'Jump + fire',crouch:'Crouch',jab:'Jab',cross:'Cross','front-kick':'Front kick','round-kick':'Round kick',hurt:'Hit reaction',charge:'Charge',melee:'Melee',death:'Destruction',patrol:'Patrol',rush:'Rush'};
  ...
  function select(item){
    ...
    if(model){const link=document.getElementById('character-workshop-link'),name=item.enemy?CADEnemies.entries.find(entry=>entry.id===item.enemy).name:item.jane?'Jane':'Jessie';link.href=item.enemy?'enemies.html#'+item.enemy:item.jane?'jane.html':'jessie.html';link.textContent=`Compare big + mini ${name} on one grid →`;}
