...
- Jessie and Jane now have approved-family masters in `jessie.js` and `jane.js`.
  ...
  Compare both families together in `jane.html`; enemy art and the explicitly
  labeled Jane / Courier outfit remain legacy studies until their own passes.
...
- New masters are available in the workshops and Concept Art. Existing gameplay
  retains legacy collision/art sizing until its separate integration is requested.
