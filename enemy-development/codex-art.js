...
(() => {
  function draw(ctx,item,t=0,state='idle',face=1){
    if(item.enemy){CADEnemies.draw(ctx,item.enemy,item.enemySize,20,40,{state,time:t,facing:face});return;}
    if(item.jane){CADJane.draw(ctx,item.jane,20,40,{state,time:t,facing:face});return;}
    if(item.jessie){CADJessie.draw(ctx,item.jessie,20,40,{state,time:t,facing:face});return;}
  ...
  const entries=[
    ['jessie-mini','Characters','Jessie / Mini','Approved Proposal A: messy brown hair, brown eye, red leather jacket, rolled cuffs, forearm tattoo, blue jeans and combat boots. 32px tall in a native 48 × 48 cell; all ten actions.',{hero:'jessie',jessie:'mini'}],
    ['jane-mini','Characters','Jane / Mini','Approved Proposal A: dark red side ponytail, blue eye, purple and lime jacket, choker, torn jeans and combat boots. 32px tall in a native 48 × 48 cell; all fourteen actions.',{hero:'jane',jane:'mini'}],
