 ...
 * Derived from baseline/jane.js; native contour edits, no bitmap resizing.
...
(() => {
  ...
  function big(c,state,f){
    ...
    function leg(shape,back){const h=shape.hip,k=shape.knee,a=shape.ankle;
      ...
      if(!back){
        patch(p,'4',h,k,.18,.32,3,4);
        // Two ragged openings, with skin beneath and pale denim threads.
        patch(p,'0',h,k,.70,.87,-3,3);patch(p,'6',h,k,.73,.84,-2,2);
        patch(p,'7',h,k,.75,.80,-1,2);patch(p,'f',h,k,.70,.73,-3,1);
        patch(p,'0',k,a,.20,.32,-2,2);patch(p,'7',k,a,.23,.29,-1,2);
        patch(p,'4',k,a,.19,.22,-2,2);
      }
      boot(shape,back);
    }
    ...
    function arm(shoulder,hand,back=false,knownElbow=null,punch=false){const s=point(...shoulder),h=point(...hand),e=knownElbow?point(...knownElbow):elbow(s,h),n=normal(s,e),m=normal(e,h),at=(a,b,t,w)=>offset(mix(a,b,t),normal(a,b),w);
      ...
      if(!back){p('d',[lit(s,e,.05,0),lit(s,e,.13,2),lit(s,e,.26,2),lit(s,e,.19,0)]);
        patch(p,'e',s,e,.34,.43,-2,2);patch(p,'b',s,e,.75,.83,-2,2);}
    ...
    // Exposed neck with black choker and a single lime stud; no armor proposed.
    body('0',[[0,-79],[5,-79],[6,-77],[2,-77]]);
    body('e',[[3,-79],[4,-79],[4,-78],[3,-78]]);
    ...
    if(state==='walk'||state==='run')travelArm(false,state==='run');
      ...
      r('0',8,-3,26,8);r('1',9,-2,24,6);r('3',10,-2,22,2);r('4',11,-2,18,1);
      r('0',33,-2,5,5);r('3',34,-2,3,1);r('b',21,0,12,2);r('d',26,0,6,1);
      r('0',17,-5,5,2);r('4',18,-5,3,1);
      p('0',[[10,4],[15,4],[13,10],[9,10]]);p('2',[[11,5],[14,5],[12,9],[10,9]]);
      r('0',23,3,6,3);r('2',24,4,4,1);r('7',8,3,4,3);r('6',24,3,4,2);r('7',25,3,2,1);
  ...
  window.CADJaneProposal={palette,specs,states:['idle'],durations:{idle:CADJaneMotion.timings.idle},draw};
})();
