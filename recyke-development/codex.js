(() => {
  ...
  const aliases={'jessie-detailed':'jessie-mini',jessie:'jessie-mini','jessie-siege':'jessie-mini','jane-detailed':'jane-mini',jane:'jane-mini',...Object.fromEntries(CADEnemies.entries.map(entry=>[entry.id,entry.id+'-mini']))};
  const motionNames={stand:'Stand',idle:'Ready',walk:'Walk',run:'Run',fire:'Fire', 'run-fire':'Run + fire',jump:'Jump','jump-fire':'Jump + fire',crouch:'Crouch study','crouch-idle':'Crouch hold','crouch-walk':'Crouch walk','crouch-fire':'Crouch + fire','crouch-walk-fire':'Crouch walk + fire',jab:'Jab',cross:'Cross','front-kick':'Front kick','round-kick':'Round kick',hurt:'Hit reaction',charge:'Charge',melee:'Melee',death:'Destruction',patrol:'Patrol',rush:'Rush'};
  const enemyModels=Object.fromEntries(CADEnemies.entries.map(entry=>[entry.id,CADEnemies.adapter(entry.id)]));
  const characterModel=item=>item.enemy?enemyModels[item.enemy]:item.jane?CADJane:item.jessie?CADJessie:null;
  const characterSize=item=>item.enemySize||item.jane||item.jessie;
  let current=entries[0],time=0,last=performance.now(),paused=false,facing=1,stripKey='';
  const groups=[...new Set(entries.map(e=>e.group))];

  function icon(canvas,group,entry){
    const c=canvas.getContext('2d'),r=(color,x,y,w,h)=>{c.fillStyle=color;c.fillRect(x,y,w,h);};
    r('#152435',0,0,32,32);
    // These are authored 32px UI drawings, never resized world sprites.
    if(group==='Mechs'){
      const id=entry.enemy,armor=id==='scrapper'?'#a47954':id==='bastion'?'#748b82':'#647f92',light=id==='scrapper'?'#d4ad7b':'#b1c4bb';
      r('#101923',11,5,11,8);r(armor,12,6,9,7);r(light,12,6,9,1);r('#e9ac74',17,9,5,2);
      r('#101923',6,13,19,11);r(armor,8,14,15,8);r(light,8,14,13,1);r('#344755',13,18,7,4);r(armor,9,23,5,5);r(armor,19,23,5,5);r(light,8,28,7,2);r(light,18,28,8,2);
      if(id==='bastion'){r('#263d45',4,3,14,6);r(light,5,3,12,1);for(let x=6;x<17;x+=4)r('#d5a37b',x,5,2,2);r('#101923',3,15,8,13);r(armor,4,16,6,11);r(light,4,16,1,11);r('#d5a37b',6,21,3,2);}
      else if(id==='scrapper'){r(armor,23,16,4,7);r(light,23,21,6,2);r(light,24,21,2,6);r(light,29,20,2,6);r('#a47954',4,12,6,9);r('#d4ad7b',4,12,6,2);}
      else{r('#263d45',22,14,8,5);r(light,23,14,7,1);r('#79d7d1',28,16,3,1);}
      return;
    }
    if(group==='Drones'){
      const id=entry.enemy,optic=id==='collector'?'#e7b47b':'#e58b8b';
      if(id==='manta'){r('#365064',3,10,26,6);r('#7896a4',5,9,22,3);r('#aec5c1',8,9,16,1);r('#456276',1,13,7,5);r('#456276',24,13,7,5);r('#647f92',4,17,6,3);r('#647f92',22,17,6,3);}
      else{r('#536f81',5,14,23,4);for(const x of [5,25]){r('#1b3144',x-2,10,4,11);r('#aec5c1',x-4,9,8,1);r('#79d7d1',x-1,21,2,2);}}
      r('#101923',11,9,10,14);r('#7896a4',12,10,8,11);r('#aec5c1',12,10,8,1);r('#263d52',13,13,7,6);r(optic,14,15,5,2);
      if(id==='collector'){r('#b08b61',12,22,2,6);r('#b08b61',19,22,2,6);r('#e7b47b',9,27,5,2);r('#e7b47b',19,27,5,2);}else{r('#263d45',14,22,4,4);r('#e58b8b',15,25,2,2);}
      return;
    }
    const color=group==='Worlds'?'#8da9b7':group==='Characters'?'#d29b82':'#758fa5';
    if(group==='Worlds'){r(color,5,12,8,16);r(color,16,5,11,23);for(let y=8;y<25;y+=5)r('#b5c6c3',18,y,6,2);}
    else if(group==='Tools'){r(color,3,16,26,4);r('#78dfd1',6,21,6,2);r('#78dfd1',21,21,5,2);}
    else if(group==='Weapons'){r(color,5,12,23,6);r(color,10,18,4,8);r('#78dfd1',19,13,8,1);}
    else{r(color,10,6,12,9);r(color,7,16,18,10);r('#79d7d1',12,9,8,2);r('#79d7d1',9,18,14,2);}
  }
  ...
  function select(item){
    ...
    motion.replaceChildren(...states.map(state=>{
      ...
      option.textContent=model?(motionNames[state]||state):state==='idle'&&item.detailed?'Fighting stance':state==='run'&&item.group==='Tools'?'Boost':state.charAt(0).toUpperCase()+state.slice(1);
      return option;
    }));
    if(model)motion.value=model.states.includes('run')?'run':model.states.includes('patrol')?'patrol':model.states[0];else if(item.detailed)motion.value='walk';
    document.getElementById('walk-review').hidden=!item.detailed;
    document.getElementById('jessie-review').hidden=!model;
    if(model){const link=document.getElementById('character-workshop-link'),name=item.enemy?CADEnemies.entries.find(entry=>entry.id===item.enemy).name:item.jane?'Jane':'Jessie';link.href=item.enemy?'enemies.html#'+item.enemy:item.jane?'jane.html':'jessie.html';link.textContent=`Inspect mini ${name} on one grid →`;}
    ...
    // Heroes and enemies share this presentation cell and zoom. The mini
    // keeps its native 48px source geometry inside the larger review canvas.
    canvas.width=model?48:item.group==='Worlds'?480:128;
    canvas.height=model?48:item.group==='Worlds'?270:80;
  ...
  function category(group,preferred){
    ...
    for(const entry of list){
      ...
      const thumb=document.createElement('canvas');thumb.width=thumb.height=32;thumb.setAttribute('aria-hidden','true');icon(thumb,group,entry);
  ...
  const fromHash=()=>{const hash=location.hash.slice(1).replace(/-big$/, '-mini');return entries.find(e=>e.id===(aliases[hash]||hash));};
