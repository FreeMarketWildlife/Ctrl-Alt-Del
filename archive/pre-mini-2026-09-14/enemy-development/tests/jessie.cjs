...
(async () => {
  ...
  try {
    ...
    const result = await page.evaluate(() => {
      ...
      // Check the finished raster, including diagonal native-pixel connections.
      // The crown must belong to the main body; detached effects are permitted.
      const crownComponentSize=(data,width)=>{
        let first=-1;for(let i=3;i<data.length;i+=4)if(data[i]){first=(i-3)/4;break;}
        if(first<0)return 0;
        const seen=new Uint8Array(data.length/4),queue=[first],height=data.length/4/width;seen[first]=1;
        for(let read=0;read<queue.length;read++){
          ...
          for(let dy=-1;dy<=1;dy++)for(let dx=-1;dx<=1;dx++){
            ...
            const next=ny*width+nx;if(!seen[next]&&data[next*4+3]){seen[next]=1;queue.push(next);}
          }
        }
        return queue.length;
      };
        ...
        for(const state of A.states) {
          ...
          for(let f=0;f<A.durations[state].length;f++) {
            for(const facing of [1,-1]) {
              ...
              if(size==='big'&&crownComponentSize(data,512)<=count/2)failures.push(`${size}/${state}/${f}/${facing}: head is disconnected from the main body`);
    ...
    console.log('PASS: all poses, both directions, connected head/body, cell bounds, palette, opaque pixels, planted feet, run/fire independence, timing, exports, desktop/mobile pixel grids.');
