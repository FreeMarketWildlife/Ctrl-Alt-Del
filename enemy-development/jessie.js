...
(() => {
  ...
  const columns = 12;
  const states = ['idle','walk','run','fire','run-fire','jump','jump-fire','crouch','jab','hurt','crouch-idle','crouch-walk','crouch-fire','crouch-walk-fire'];
  ...
  function low(c,state,f){
    const {rect:r,poly:p,disk,stamp}=raster(c),pose=CADJessieMotion.sample('mini',state,f);
    function leg(q,back){
    ...
    // Red leather torso folds forward, with lapel, cuff and blue jeans.
    p('0',[[-6,-7],[-3,-11],[2,-13],[6,-11],[5,-6],[0,-3],[-5,-4]]);
    p('a',[[-5,-7],[-2,-10],[2,-12],[5,-10],[4,-7],[0,-4],[-4,-5]]);
    p('b',[[-4,-8],[-1,-10],[2,-11],[3,-8],[-1,-5],[-4,-5]]);
    r('c',-1,-10,3,2);r('d',-1,-10,1,1);r('0',-5,-5,6,2);r('e',-1,-5,2,1);
    r('7',2,-13,3,4);stamp(['..55.65..','.5676665.','556655885','.55680085','..688f680','..6788880','...678650','....686..'],-1,-18);
    ...
    r('0',4,-11,4,5);r('b',4,-10,3,3);r('c',4,-10,1,2);
    r('0',5,-7,8,3);r('8',6,-7,5,1);r('1',11,-8,3,3);
    r('a',5,-8,2,1);r('5',8,-7,1,1);
    r('0',8+recoil,-10,10,4);r('3',9+recoil,-10,8,1);r('c',11+recoil,-9,6,1);
    r('0',10+recoil,-6,2,3);r('8',10+recoil,-7,2,1);
    if(shot){r('e',18,-10,2,3);r('f',18,-9,2,1);}
  }
  ...
  function draw(c,size,x,feet,{state='idle',time=0,frame=null,facing=1,silhouette=false}={}){if(size!=='mini')throw new RangeError('Large models are archived; use mini.');
    ...
    (state.startsWith('crouch-')?low:mini)(c,state,frame??frameAt(state,time),time);c.restore();
  }
  ...
  function metadata(){return {name:'Jessie',sourcePixelWorldUnits:1,palette,sizes:specs,columns,rows:states,timingMs:durations,frameCounts:Object.fromEntries(states.map(s=>[s,durations[s].length])),loops:Object.fromEntries(states.map(s=>[s,['idle','walk','run','run-fire','crouch-idle','crouch-walk','crouch-walk-fire'].includes(s)])),contactFrames:CADJessieMotion.contactFrames,framePadding:'Repeat final authored pose to fill 12 columns'};}
