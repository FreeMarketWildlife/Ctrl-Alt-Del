...
assert.notEqual(configs.big.platforms.length,0);
const bigRatios=configs.big.platforms.slice(1).map((p,i)=>p.x/configs.mini.platforms[i+1].x);
assert(new Set(bigRatios.map(x=>x.toFixed(3))).size>5,'mini map is separately authored, not a global rescale');
