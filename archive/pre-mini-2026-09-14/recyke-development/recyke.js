...
(() => {
  ...
  let size = /^(mini|small)$/.test(params.get('size')) ? 'mini' : 'big';
  ...
  const heldControls = new Set(['left', 'right', 'fire']);
  const mapping = { KeyA: 'left', ArrowLeft: 'left', KeyD: 'right', ArrowRight: 'right',
    KeyW: 'jump', ArrowUp: 'jump', Space: 'fire', KeyK: 'fire', KeyE: 'melee', KeyQ: 'swap' };
  ...
  function syncChoices() {
    ...
    setText('family-note', size === 'big' ? 'Big version / 96px characters' : 'Small version / 32px characters');
    setText('hero-name', hero.toUpperCase());
    canvas.setAttribute('aria-label', `${size === 'big' ? 'Big' : 'Small'} ${hero === 'jane' ? 'Jane' : 'Jessie'} in Recyke. Move A/D or arrows, jump W/Up, fire Space/K, melee E, swap Q, pause P.`);
    updateRoute();
  }
  ...
  function syncOverlay() {
    ...
    const copy = mode === 'won'
      ? ['LINE 7 / REACHED', 'Shift complete.', `You made it through the Recyke. ${latest.kills || 0} machines disabled. Try the other size or switch hero for your next run.`, 'Play again', 'R to play again']
      : mode === 'dead'
  ...
  function input() {
    const result = { left: false, right: false, jump: false, fire: false, melee: false, swap: false, autoFire: $('auto-fire').checked };
  ...
  for (const button of document.querySelectorAll('[data-size]')) button.addEventListener('click', () => {
    ...
    setText('announcements', `${size === 'big' ? 'Big' : 'Small'} version. A new shift begins.`);
  });
