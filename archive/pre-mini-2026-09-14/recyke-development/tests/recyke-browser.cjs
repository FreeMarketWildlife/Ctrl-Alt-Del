...
async function open(page, size = 'big', hero = 'jane') {
...
(async () => {
  ...
  for (const size of ['big', 'mini']) for (const hero of ['jessie', 'jane']) { await checkKeyboard(page, size, hero); console.log(`Keyboard PASS: ${size}/${hero}`); }
  await checkLinks(page);
  const playthroughs = [];
  for (const size of ['big', 'mini']) { playthroughs.push(await checkCompletion(page, size, size === 'big' ? 'jane' : 'jessie')); console.log(`Completion PASS: ${size}`); }
  for (const size of ['big', 'mini']) { await checkDeath(page, size); console.log(`Defeat and retry PASS: ${size}`); }
  ...
  console.log(`PASS: both heroes and sizes through keyboard, pause/resume, blur/restart; multi-touch movement/fire/cancel; ${results.length} native-grid viewport combinations; valid navigation links.`);
