(() => {
  ...
  const pcModeButton = document.getElementById("pcMode");
  ...
  setPCMode(pcMode);
  window.addEventListener('pointerdown',e=>{if(e.pointerType==='touch'&&pcMode)setPCMode(false);else if(e.pointerType==='mouse'&&!pcMode)setPCMode(true);},{capture:true});
  window.addEventListener('keydown',e=>{if(running&&!pcMode&&['w','a','s','d','ArrowLeft','ArrowRight',' '].includes(e.key))setPCMode(true);},{capture:true});
  async function landscape(){
    if(pcMode||!screen.orientation?.lock)return;
    try{if(!document.fullscreenElement&&shell.requestFullscreen)await shell.requestFullscreen();if(running)await screen.orientation.lock('landscape');}catch(_){}
  }

  window.addEventListener("mousemove", e => {
    if (running && currentMode==='fps' && pcMode && e.target===canvas && e.movementX) touch.look += e.movementX * .004;
  }, { passive:true });
  ...
  function makeControls({ look=false, jump=true, vertical=false } = {}) {
    ...
    const moveStick=e=>{
      ...
      let dx=e.clientX-(box.left+box.width/2),dy=(look||vertical)?e.clientY-(box.top+box.height/2):0;
  ...
  function launch(mode) {
    ...
    if(mode==="chase") {makeControls({jump:false,vertical:true});active=CADFlightLevel.create({ctx,input,touch,audio,showMessage,hideMessage,controls,gameTitle,gameMeta,releaseInputs});}
  ...
  function createPlatformer(){
    ...
    let character="jessie";
    const characterButton=document.createElement("button");
    characterButton.className="touch-btn";characterButton.style.cssText="position:absolute;right:12px;top:48px;width:auto;padding:8px;pointer-events:auto;font-size:10px";
    characterButton.textContent="PLAY AS JANE";controls.appendChild(characterButton);
    characterButton.onclick=()=>{character=character==="jessie"?"jane":"jessie";characterButton.textContent=character==="jessie"?"PLAY AS JANE":"PLAY AS JESSIE";};
    let p, bullets, enemyBullets, enemies, particles, score, spawnClock, elapsed, dead;
    const platforms=CADWorld.platforms;

    function reset(){
      p={x:60,y:198,w:10,h:35,vx:0,vy:0,onGround:false,facing:1,hp:100,shot:0,punch:0,punchLock:false,inv:0,landing:0,runTime:0};
      bullets=[];enemyBullets=[];enemies=[];particles=[];score=0;spawnClock=.5;elapsed=0;dead=false;hideMessage();
    }

    function rectHit(a,b){return a.x<b.x+b.w&&a.x+a.w>b.x&&a.y<b.y+b.h&&a.y+a.h>b.y;}
    function spawnEnemy(){
      const roll=Math.random();const kind=roll<.22?"mech":roll<.5?"drone":"walker";
      enemies.push(kind==="mech"?{kind,x:492,y:195,w:27,h:39,vx:-13,hp:7,shot:2.2,flash:0}:kind==="walker"?{kind,x:492,y:210,w:11,h:24,vx:-20-Math.random()*12,hp:2,shot:.7+Math.random()*1.3,flash:0}:{kind,x:492,y:145+Math.random()*55,w:16,h:9,vx:-25-Math.random()*16,hp:1,shot:.8+Math.random()*1.1,phase:Math.random()*8,flash:0});
    }
    ...
    function damage(amount){if(p.inv>0||dead)return;p.hp-=amount;p.inv=.55;audio(120,.09,"sawtooth",.04);burst(p.x+4,p.y+10,"#ff5269",8);if(p.hp<=0){dead=true;showMessage("SIGNAL LOST",`${character.toUpperCase()} went down. Robots neutralized: ${score}.`,"RETRY",reset);}}

    function update(dt){
      if(dead)return; elapsed+=dt;p.landing=Math.max(0,p.landing-dt);p.shot=Math.max(0,p.shot-dt);p.punch=Math.max(0,p.punch-dt);p.inv=Math.max(0,p.inv-dt);
      const keyboardMove=(input.has("left")||input.has("strafeLeft")?-1:0)+(input.has("right")||input.has("strafeRight")?1:0);
      const move=Math.abs(touch.x)>.12?touch.x:keyboardMove;p.vx=move*82;if(Math.abs(p.vx)>5&&p.onGround)p.runTime+=Math.abs(p.vx)*dt/42*(8/14);else if(Math.abs(p.vx)<=5)p.runTime=0;if(Math.abs(move)>.08)p.facing=Math.sign(move);
      if(input.has("up")&&p.onGround){p.vy=-165;p.onGround=false;audio(260,.05,"square",.02);input.delete("up");}
      if(input.has("fire")&&p.shot<=0){p.shot=.19;const bx=p.x+p.w/2+(p.facing>0?20:-26);bullets.push({x:bx,y:p.y+p.h-24,w:6,h:2,vx:p.facing*235});audio(880,.04,"square",.025);}
      ...
      const wasGrounded=p.onGround;const oldBottom=p.y+p.h;p.y+=p.vy*dt;p.onGround=false;
      for(const plat of platforms){if(p.vy>=0&&p.x+p.w>plat.x&&p.x<plat.x+plat.w&&oldBottom<=plat.y+3&&p.y+p.h>=plat.y){if(!wasGrounded&&p.vy>40)p.landing=.12;p.y=plat.y-p.h;p.vy=0;p.onGround=true;}}
      ...
      for(const b of enemyBullets){b.x+=b.vx*dt;b.y+=b.vy*dt;if(rectHit(b,p)){b.dead=true;damage(b.damage||12);}}
      for(const e of enemies){
        e.x+=e.vx*dt;e.shot-=dt;e.flash=Math.max(0,e.flash-dt);
        if(e.kind==="drone")e.y+=Math.sin(elapsed*3+e.phase)*7*dt;
        if(e.shot<=0&&e.x>120){const dx=(p.x-e.x),dy=(p.y+8-e.y);const len=Math.hypot(dx,dy)||1;enemyBullets.push({x:e.x,y:e.y+5,w:e.kind==="mech"?7:4,h:e.kind==="mech"?4:2,damage:e.kind==="mech"?22:12,vx:dx/len*(e.kind==="mech"?100:82),vy:dy/len*(e.kind==="mech"?100:82)});e.shot=e.kind==="mech"?2.4:1.2+Math.random()*1.3;e.flash=.08;audio(190,.03,"square",.012);}
        if(rectHit(e,p)){if(e.kind!=="mech")e.dead=true;damage(e.kind==="mech"?28:20);}
    ...
    function drawRobot(e){if(e.kind==="mech"){CADWorld.mech(ctx,e.x+e.w/2,e.y+e.h,{time:elapsed,facing:-1,flash:e.flash>0,charging:e.shot<.5});return;}const flash=e.flash>0;ctx.fillStyle=flash?"#fff":"#14131b";if(e.kind==="walker"){ctx.fillRect(e.x+2,e.y,7,6);ctx.fillRect(e.x,e.y+6,11,10);ctx.fillRect(e.x+1,e.y+16,3,8);ctx.fillRect(e.x+7,e.y+16,3,8);ctx.fillStyle="#ff5269";ctx.fillRect(e.x+4,e.y+2,3,2);ctx.fillStyle="#8e62ff";ctx.fillRect(e.x-4,e.y+8,5,3);}else{ctx.fillRect(e.x,e.y+2,16,5);ctx.fillRect(e.x+5,e.y,6,9);ctx.fillStyle="#ff5269";ctx.fillRect(e.x+7,e.y+3,2,2);ctx.fillStyle="#8e62ff";ctx.fillRect(e.x-3,e.y+4,3,1);ctx.fillRect(e.x+16,e.y+4,3,1);}}

    function draw(){
      CADWorld.draw(ctx,elapsed);
      ...
      enemies.forEach(drawRobot);particles.forEach(q=>{ctx.fillStyle=q.color;ctx.fillRect(q.x|0,q.y|0,2,2)});CADCharacters.draw(ctx,character,p.x+p.w/2,p.y+p.h,{facing:p.facing,firing:p.shot>.14,jumpPhase:p.onGround?(p.landing>0?.9:null):Math.max(.125,Math.min(.8,.125+(p.vy+165)/380*.8)),time:p.punch>0?(.18-p.punch)/.18:p.onGround&&Math.abs(p.vx)>5?p.runTime:p.shot>.08?(.19-p.shot)*4:elapsed,state:p.inv>0?"hurt":p.punch>0?"punch":!p.onGround||p.landing>0?"jump":Math.abs(p.vx)>5?"run":p.shot>.08?"fire":"idle"});
      ctx.fillStyle="rgba(4,3,9,.82)";ctx.fillRect(7,27,128,18);ctx.fillStyle="#fff";ctx.font="7px monospace";ctx.fillText(character.toUpperCase()+" // HP",12,36);ctx.fillStyle="#30131f";ctx.fillRect(61,31,66,7);ctx.fillStyle=p.hp>35?"#39f2df":"#ff5269";ctx.fillRect(61,31,Math.max(0,66*p.hp/100),7);ctx.fillStyle="#fff";ctx.fillText(`ROBOTS ${score}`,390,37);
    }
