...
const states=['stand','idle','walk','run','fire','run-fire','jump','jump-fire','crouch','jab','cross','front-kick','round-kick','hurt','crouch-idle','crouch-walk','crouch-fire','crouch-walk-fire'];
...
for(const size of ['mini']){
  ...
  for(const [state,holds] of Object.entries(motion.timings)){
    ...
    for(let frame=0;frame<holds.length;frame++){
      ...
      if(state.includes('walk'))assert(pose.near.contact||pose.far.contact,`${label}: walk lost support`);
      ...
      assert.deepEqual(pose.hair[0],[anatomy.hair.root[0]+(pose.headOffset?.[0]??pose.lean),anatomy.hair.root[1]+(pose.headOffset?.[1]??pose.bob)],`${label}: hair detached from head`);
  ...
  for(const state of ['walk','run','crouch-walk','crouch-walk-fire']){
    assert.equal(motion.timings[state].length,12,`${state}: expected twelve authored poses`);
    assert(motion.timings[state].every(ms=>ms===(state.includes('walk')?80:50)),`${state}: cadence diverges from shared character standard`);
    ...
    for(let frame=0;frame<12;frame++){
      ...
      for(const side of sides){
        ...
        if(state.startsWith('crouch-walk')&&a.contact&&b.contact)assert.equal(a.ankle[0]-b.ankle[0],2,'crouch support must match 25px/s over an 80ms hold');
  ...
  for(const [moving,firing] of [['run','run-fire'],['jump','jump-fire'],['crouch-walk','crouch-walk-fire']]){
...
console.log(`Jane motion: ${checkedPoses} poses; 18 actions, fixed bones, planted kicks, native hair links, waving run hair, and shared firing cadence pass. Max bone error ${maxBoneError.toFixed(3)}px; max hair link error ${maxHairError.toFixed(3)}px.`);
