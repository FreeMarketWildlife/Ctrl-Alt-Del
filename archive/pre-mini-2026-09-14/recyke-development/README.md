...
## Recyke starter level — First Shift

Play [Big](recyke.html?size=big) or [Small](recyke.html?size=mini) through the Recyke
sorting yard to Line 7. Both versions feature the approved Jessie, Jane and six enemy
archetypes, with separately placed scenery, platforms and physics for their native
96px / 32px character heights. An elevated public train, sorting machinery and worn
industrial buildings frame the route. Collect salvage, use cover, secure the repair
checkpoint, and disable the Bastion to open the depot exit.

Choose Jessie or Jane in the toolbar; Q swaps during play. Use A/D or arrows to move,
W/up to jump, Space/K to fire, E to melee, and P/Escape to pause. Touch buttons support
movement and attacks together. Optional Auto fire is available. Changing size starts
a fresh route; defeat offers checkpoint retry. The level is also linked from the
Episodes menu, Concept Art and all three model workshops.

See [the level notes](RECYKE-LEVEL.md), [both native worlds together](docs/review/recyke-big-small.png)
and [the gameplay animation](docs/review/recyke-playthrough.gif). Run
`node tests/recyke-level.cjs` for simulation checks; the native-art and browser suites
are `tests/recyke-art.cjs` and `tests/recyke-browser.cjs`.
...
Enemy verification: run `node tests/enemy-mech-motion.cjs` and
`node tests/enemies.cjs` (Playwright) for ground joints, every enemy pose and
direction, native dimensions, exports, workshop/catalog controls and unchanged hero
pixels. Set `UPDATE_ASSETS=1` to regenerate `assets/enemies/`. Review media can be
regenerated with `node tests/render-enemy-review.cjs` using canvas and sharp.
